...
export const Register = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:5423'}/api/auth/register`, {
        method: 'POST',
        headers: {
  ...
  const handleGoogleAuth = () => {
    window.location.href = `${process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:5423'}/api/auth/google`;
  };
  ...
  return (
    <Container className="h-screen max-w-lg mx-auto flex flex-col items-center justify-center">
      ...
      <form className="w-full my-4" onSubmit={handleSubmit}>
        ...
        <Button variant="muted" type="submit" className="w-full py-3" disabled={isLoading}>
          <span className="text-sm">{isLoading ? 'Signing up...' : 'Sign up'}</span>
        </Button>
      </form>
